...
void events_init()
{
    ...
    logger_init(&_event_table._logger);
...
{
    log_info(&_event_table._logger, "Attempting to create event %s.\n", event_name);
    ...
    if (_event_table._root_event == NULL)
    {
        ...
        log_info(&_event_table._logger, "Create event %s. Succeeded - inserted as root.\n", event_name);
    ...
    {
        log_info(&_event_table._logger, "Create event %s. Failed - an event already exists with that name.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return 1;
    }
    ...
    log_info(&_event_table._logger, "Create event %s. Succeeded.\n", event_name);
...
{
    log_info(&_event_table._logger, "Attempting to remove event %s.\n", event_name);

    // If root is NULL, there are no events to remove.
    if (_event_table._root_event == NULL)
    {
        log_info(&_event_table._logger, "Remove event %s. Failed - _event_table empty.\n", event_name);
        return 1;
    }
    ...
    if (!strcmp(_event_table._root_event->event_name, event_name))
    {
        ...
        if (_event_table._root_event->_next_event != NULL)
        {
            ...
            log_info(&_event_table._logger, "Remove event %s. Succeeded - re-assigned root.\n", event_name);
        }
        // Else, set root to NULL.
        else
        {
            _event_table._root_event = NULL;
            log_info(&_event_table._logger, "Remove event %s. Succeeded - _event_table empty.\n", event_name);
        }
    ...
    {
        log_info(&_event_table._logger, "Remove event %s. Failed - event not found.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return 2;
    }
    // Else, event is found.
    ...
    pthread_mutex_unlock(&_event_table._mutex);
    log_info(&_event_table._logger, "Remove event %s. Succeeded.\n", event_name);
    return 0;
}
...
{
    log_info(&_event_table._logger, "Attempting to trigger event %s.\n", event_name);
    ...
    // Event_table empty.
    case 1:
        log_info(&_event_table._logger, "Trigger event %s. Failed - _event_table empty.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return rc;
    // Event not found.
    case 2:
        log_info(&_event_table._logger, "Trigger event %s. Failed - event not found.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return rc;
    // Event found.
    default:
    ...
    log_info(&_event_table._logger, "Trigger event %s. Succeeded.\n", event_name);
    pthread_mutex_unlock(&_event_table._mutex);
    return 0;
}
...
{
    log_info(&_event_table._logger, "Attempting to subscribe to event %s.\n", event_name);
    ...
    // Event_table empty.
    case 1:
        log_info(&_event_table._logger, "Subscribe to event %s. Failed - _event_table empty.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return rc;
    // Event not found.
    case 2:
        log_info(&_event_table._logger, "Subscribe to event %s. Failed - event not found.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return rc;
    // Event found.
    default:
    ...
    if (event->_root_subscription == NULL)
    {
        ...
        log_info(&_event_table._logger, "Subscribe to event %s. Succeeded - inserted as root.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return 0;
    }
    ...
    {
        log_info(&_event_table._logger, "Subscribe to event %s. Failed - the handler is already subscribed to the event.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return 3;
    }
    ...
    log_info(&_event_table._logger, "Subscribe to event %s. Succeeded.\n", event_name);
    pthread_mutex_unlock(&_event_table._mutex);
    return 0;
}
...
{
    log_info(&_event_table._logger, "Attempting to unsubscribe to event %s.\n", event_name);
    ...
    {
    case 1:
        log_info(&_event_table._logger, "Unsubscribe to event %s. Failed - _event_table empty.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return rc;
    case 2:
        log_info(&_event_table._logger, "Unsubscribe to event %s. Failed - event not found.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return rc;
    default:
        break;
    ...
    if (event->_root_subscription == NULL)
    {
        log_info(&_event_table._logger, "Unsubscribe to event %s. Failed - subscriptions empty.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return 3;
    }
    ...
    if (event->_root_subscription->_handler == handler)
    {
        ...
        if (event->_root_subscription->_next_subscription != NULL)
        {
            ...
            log_info(&_event_table._logger, "Unsubscribe to event %s. Succeeded - re-assigned root.\n", event_name);
        }
        // Else, set root to NULL.
        else
        {
            event->_root_subscription = NULL;
            log_info(&_event_table._logger, "Unsubscribe to event %s. Succeeded - subscriptions empty.\n", event_name);
        }
    ...
    {
        log_info(&_event_table._logger, "Unsubscribe to event %s. Failed - the handler is not subscribed.\n", event_name);
        pthread_mutex_unlock(&_event_table._mutex);
        return 3;
    }
    ...
    pthread_mutex_unlock(&_event_table._mutex);
    log_info(&_event_table._logger, "Unsubscribe to event %s. Succeeded.\n", event_name);
    return 0;
}
